...
- 冒頭付近で「今日はtelegramへのメモ投稿がなかった」旨に一文だけさらっと触れる。エラーや異常があったかのような書き方はせず、「特に記録することもなく過ぎた一日だった」のような、日記として自然な軽いトーンにする
...
- メモに「昨日」「一昨日」「先週」「さっき」「明日」「来週」などの時間表現がある場合は、その時間関係をそのまま保って記事にする。{today_str}の出来事に置き換えない
  - 過去を指すメモは過去の出来事として書く。悪い例：「昨日は久しぶりに走った」というメモを「今日は久しぶりに走った」と書く
  - 未来を指すメモは予定・見込みとして書き、済んだ出来事にしない。悪い例：「明日は病院」というメモを「病院に行った」と書く
  - 本文でも「昨日」「明日」などの相対表現をそのまま使ってよい。具体的な日付に言い換える必要はない
