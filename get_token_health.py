"""
Google Health API OAuth2 リフレッシュトークン取得スクリプト（一度だけ実行）
Fitbit Air のデータは Google Health アプリ経由で同期される。

【必要なスコープ】
  googlehealth.activity_and_fitness.readonly  - 歩数・運動データ
  googlehealth.sleep.readonly                 - 睡眠データ

【事前確認】
  Google Cloud Console > APIとサービス > ライブラリ で
  「Google Health API」が有効になっていること。
  OAuth 同意画面のテストユーザーに nadaryu@gmail.com が追加済みであること。
  （カレンダーと同じプロジェクト・クライアントIDを使用可能）
"""
import urllib.parse
...
try:
    ...
    print("\n↑ このトークンを GitHub Secrets の GHEALTH_REFRESH_TOKEN に登録してください。")
    print("  クライアントID / SECRET は GCAL と同じものを使用した場合、")
    print("  GHEALTH_CLIENT_ID / GHEALTH_CLIENT_SECRET の登録は不要です。")
